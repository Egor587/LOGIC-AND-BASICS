...
//Функция добавления ребра в список смежности (с поддержкой петель)
void addEdge(int src, int dest) {
    // Добавление в матрицу смежности
    graph_matrix[src][dest] = 1;
    // Для неориентированного графа добавляем обратное ребро (кроме случая петли)
    if (src != dest) {
        graph_matrix[dest][src] = 1;
    }

    // Добавление в списки смежности
...
//Функция добавления петли (специальная функция для петель)
void addLoop(int vertex) {
    // Добавление петли в матрицу смежности
    graph_matrix[vertex][vertex] = 1;

    // Добавление петли в список смежности
    Node* newNode = (Node*)malloc(sizeof(Node));
    newNode->vertex = vertex;  // Петля - ссылка на саму себя
...
//Рекурсивная функция обхода в глубину (матрица)
void DFS_matrix_recursive(int start_vertex) {
    ...
    for (int i = 0; i < num_vertices; i++) {
        //Если есть связь и вершина еще не посещена (для петель i == start_vertex)
...
void DFS_matrix_nonRecursive(int start_vertex) {
    ...
    while (!isEmpty(stack)) {
        ...
        for (int i = 0; i < num_vertices; i++) {
            // Проверяем связь и что вершина не посещена (включая петли)
...
void DFS_adjacency_list_recursive(int vertex) {
    ...
    while (temp != NULL) {
        ...
        //Если смежная вершина еще не посещена (включая случай петли)
...
void DFS_adjacency_list_nonRecursive(int start_vertex) {
    ...
    while (!isEmpty(stack)) {
        ...
        while (temp != NULL) {
            int adjVertex = temp->vertex;// Получаем номер смежной вершины
            // Если вершина еще не посещена (включая петли)
...
int main() {
    ...
    while (1) {
        ...
        default:
            // Некорректный выбор
            printf("Ошибка: неверный выбор! Пожалуйста, выберите от 0 до 6.\n");
            break;
        }
    }
    return 0;
